// Created by DaggleWood at Github
// Based on the Earlier 'Time Signature Copy Paste' plugin by @TeeDeeY at Github.
// Started as a simple update, but ended up adding quite a few features. Still, it is all built
// directly on TeeDeeY's code, to whom foundational credit belongs.
//
// Features included:
...
MuseScore {
    menuPath: "Plugins.4.14timesigCP Buffer"

    title: "4.14 Time Sig Copy-Paste (Buffer)"
    description: "Time Sig copy/paste-overwrite 1.1 (presets + grouping)"
    version: "4.14"
...
function runPasteNonBlocking() {
    ...
    function go() {
        ...
        cursor.rewind(1)
        if (!cursor.segment) {
            statusText = "Click a destination point in the score, set REPEAT, then press Go! Paste."
            return
        }
    ...
    function loadBufferFromClipboard() {
        ...
        // Legacy semicolon
        if (testr.indexOf(";") < 0) {
            statusText = "Clipboard doesn't look like TS data yet."
            return
        }
        ...
        if (isNaN(total) || total < 1) {
            statusText = "Could not parse clipboard data."
            return
        }
        tot1++
        ...
        contentItem: ColumnLayout {
            ...
            ToolBar {
                ...
                RowLayout {
                    ...
                    Label {
                        Layout.fillWidth: true
                        text: "MuseScore Studio 4.6+ (v4.14) — presets + grouping"
                        color: uiText
                    ...
                    ColumnLayout {
                        ...
                        Text {
                            ...
                            text: "Backup your score. This plugin inserts measures plus a few extra.\n" +
                                  "Workflow:\n" +
                                  "1) Select range; 2) Run plugin; 3) Refresh/Collect if needed; 4) (Optional) Save preset; 5) Set REPEAT; 6) Click destination; 7) Go! Paste.\n" +
                                  "Presets are stored INSIDE the score file (meta tags)."
                        ...
                        RowLayout {
                            spacing: 10
                            Text { text: "REPEAT paste N times:"; color: uiText }
                        ...
                        GroupBox {
                            ...
                            ColumnLayout {
                                ...
                                Text {
                                    color: uiText
                                    text: "Clipboard uses JSON (v2) so it can include additive display + groups. Legacy semicolon data still loads."
                                    wrapMode: Text.WordWrap
                                }
                            }
                        }
